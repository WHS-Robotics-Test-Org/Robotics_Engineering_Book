...
<br>
It is important to remember which pins are which as the locations do not represent the GPIO labels.  If you forget or cannot find this diagram, use the command line in the terminal.  This is as good a place as any to start using the terminal.  With the Raspberry Pi powered on press CTRL+ALT+T to open a terminal session.  Before the Apple Lisa's introduction, all computers used something like the terminal as the interface with humans.  All computers still contain a terminal.  The Raspberry Pi OS we are using is based on the Linux Kernal so we will use Linux commands but the procedure is similar for other systems such as Windows and iOS.
